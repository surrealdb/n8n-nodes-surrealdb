...
export const createManyOperation: IOperationHandler = {
	async execute(
		...
		// Process each item to create multiple records
		for (let i = 0; i < items.length; i++) {
			try {
				...
				validateRequiredField(executeFunctions, table, 'Table', i); // Validate table after potential extraction
				
				const dataInput = executeFunctions.getNodeParameter('data', i); // Get the parameter named 'data' - could be string or array
				if (DEBUG) {
