...
export const queryRelationshipsOperation: IOperationHandler = {
	async execute(
		...
		// Get parameters
		const recordId = executeFunctions.getNodeParameter('recordId', itemIndex, '') as string;
		const direction = executeFunctions.getNodeParameter('direction', itemIndex) as string;
		...
		if (DEBUG) {
			// DEBUG: Log query
			debugLog('queryRelationships', 'Prepared query', itemIndex, preparedQuery);
		}
		...
		if (DEBUG) {
			...
			debugLog('queryRelationships', 'Raw query result', itemIndex, JSON.stringify(result));
		}
		...
		if (Array.isArray(result) && result.length > 0) {
			...
			if (splitIntoItems) {
				...
				if (!recordId) {
					...
					relationships = Array.isArray(queryResult) ? queryResult : [queryResult];
				} else if (queryResult && typeof queryResult === 'object' && 'relationships' in queryResult) {
				...
				} else if (queryResult && typeof queryResult === 'object' && 'related_records' in queryResult) {
				...
				if (returnData.length === 0) {
					returnData.push({
						json: { message: `No relationships found${recordId ? ` for ${recordId}` : ''}` },
						pairedItem: { item: itemIndex },
					});
				}
			} else {
