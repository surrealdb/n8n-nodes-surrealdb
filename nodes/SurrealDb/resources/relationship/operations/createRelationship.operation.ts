...
export const createRelationshipOperation: IOperationHandler = {
	async execute(
		...
		validateRequiredField(executeFunctions, fromRecordId, 'From Record ID', itemIndex);
		validateRequiredField(executeFunctions, relationshipType, 'Relationship Type', itemIndex);
		validateRequiredField(executeFunctions, toRecordId, 'To Record ID', itemIndex);
		...
		const properties = executeFunctions.getNodeParameter('properties.propertyValues', itemIndex, []) as IDataObject[];
		...
		if (DEBUG) {
			// DEBUG: Log query
			debugLog('createRelationship', 'Prepared query', itemIndex, preparedQuery);
		}
		...
		if (DEBUG) {
			...
			debugLog('createRelationship', 'Raw query result', itemIndex, JSON.stringify(result));
		}
