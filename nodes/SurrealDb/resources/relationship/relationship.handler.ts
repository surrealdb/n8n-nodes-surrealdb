		...
		try {
			switch (operation) {
				case 'createRelationship':
					returnData = [...returnData, ...(await createRelationshipOperation.execute(client, items, executeFunctions, i))];
					break;
				case 'deleteRelationship':
					returnData = [...returnData, ...(await deleteRelationshipOperation.execute(client, items, executeFunctions, i))];
					break;
				case 'queryRelationships':
					returnData = [...returnData, ...(await queryRelationshipsOperation.execute(client, items, executeFunctions, i))];
					break;
				default:
					throw new Error(`The operation "${operation}" is not supported for the Relationship resource!`);
			}
		} catch (error) {
