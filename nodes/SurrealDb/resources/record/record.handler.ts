		...
		try {
			switch (operation) {
				...
				case 'getRecord':
					...
					break;
				case 'updateRecord':
					returnData = [...returnData, ...(await updateRecordOperation.execute(client, items, executeFunctions, i))];
					break;
				case 'mergeRecord':
					returnData = [...returnData, ...(await mergeRecordOperation.execute(client, items, executeFunctions, i))];
					break;
				case 'deleteRecord':
					returnData = [...returnData, ...(await deleteRecordOperation.execute(client, items, executeFunctions, i))];
					break;
				case 'upsertRecord':
					returnData = [...returnData, ...(await upsertRecordOperation.execute(client, items, executeFunctions, i))];
					break;
				default:
					throw new Error(`The operation "${operation}" is not supported for the Record resource!`);
			}
		} catch (error) {
